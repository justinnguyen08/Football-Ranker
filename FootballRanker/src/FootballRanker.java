...
public class FootballRanker {
    ...
    private double printRootMeanSquareError(List<String> humanRanks,
        ...
        ArrayList<String> predictedTeamRanks = new ArrayList<>(); 
        double difference = 0;
        // add the team name to predictedTeamRanks for every path in the graph
        for (AllPathsInfo p : paths) {
        	predictedTeamRanks.add(p.getName());
        }
        // iterate through each rank of humanRanks, humanRanks already in order
        for (int rank = 0; rank < humanRanks.size(); rank++) {
        	String humanRank = humanRanks.get(rank);
        	// we have (rank + 1) because array list is 0 - based index,
        	// we would not be able to access lists correctly otherwise	
        	difference += ((rank + 1) - (predictedTeamRanks.indexOf(humanRank) + 1))
        			* ((rank + 1) - (predictedTeamRanks.indexOf(humanRank) + 1));	
        	if (showResults) {
            	System.out.println(String.format("%1$-" + PADDING + "s", humanRank) + " - actual" +
            			" rank: " + (rank + 1) + " predicted rank: " + 
            			(predictedTeamRanks.indexOf(humanRank) + 1));
        	}
        }
        // finally take the square root of the summation divided by n (humanRanks.size())
        difference = Math.sqrt(difference / humanRanks.size());
        if (showResults) {
        	System.out.println("Root Mean Square Error: " + ourFormatter.format(difference));
        }
        // formats difference to first decimal place
        return Math.round(difference * 10.0) / 10.0;
    }
