...
public class Graph {
    ...
    // values for each vertex's "scratch" value indicating
    ...
    public void dijkstra(String startName) {
        ...
        Queue<Path> q = prepDijkstra(startName);
        // while there are paths to check (unvisited vertices)
        while (!q.isEmpty()) {
        	Path currentPath = q.remove();
        	Vertex current = currentPath.dest;
        	current.scratch = VISITED;
        	// look through all edges of current vertex
	       	for (Edge e: current.adjacent) {
	       		Vertex neighborNode = e.dest;
	       		// the destination vertex has not been visited yet
	       		if (neighborNode.scratch == NOT_VISITED) {
			       	double sumOfCost = current.weightedCostFromStartVertex + 
			       			e.cost;
			       	// the sum of the cost of the current vertex and the cost of the edge is less 
			        // than the cost of the destination vertex 
		       		if (sumOfCost < neighborNode.weightedCostFromStartVertex) {
			       		// update cost of destination vertex and set the previous of the 
			       		// destination vertex to the current vertex
			       		neighborNode.weightedCostFromStartVertex = sumOfCost;
		                neighborNode.numEdgesFromStartVertex = current.numEdgesFromStartVertex + 1;
		                neighborNode.prev = current;
			       		q.add(new Path(neighborNode, neighborNode.weightedCostFromStartVertex));
			       	}
	       		}
	       	}
        }
//        printPath(startName);
    ...
    // Set up a priority queue and instance variables to find 
    // weighted shortest path algorithm from startName vertex
    // to every other Vertex we can reach.
    private Queue<Path> prepDijkstra(String startName) {
    	// same as prepForFindingUnweightedShortestPaths except we create a priority queue
    	// of paths instead of a normal queue of vertices
        currentStartVertexName = startName;
        clearAll();
        Vertex start = vertices.get(startName);
        start.weightedCostFromStartVertex = 0;
    ...
    public void findAllPaths(boolean weighted) {
    	...
    	for (Vertex current : vertices.values()) {
    		clearAll();
    		// for each vertex in the graph, call dijkstra algorithm if weighted,
    		// else find unweighted shortest path
    		if (weighted) {
    			dijkstra(current.name);
    		}
    		else {	
    			findUnweightedShortestPath(current.name);
    		}
    		// clear current path info so we increment the correct values for current vertex
    		current.clearPathInfo();
    		// for each vertex in the graph except for the current vertex being checked
			for (Vertex otherNode : vertices.values()) {
				if (!otherNode.equals(current)) {
					// if both vertices are connected
					if (otherNode.weightedCostFromStartVertex != INFINITY) {
						// increment variables appropriately based on if graph is weighted or not
						current.totalUnweightedPathLength += otherNode.numEdgesFromStartVertex;
	        			current.totalWeightedPathLength += otherNode.weightedCostFromStartVertex;
	        			current.numVertexConnected++;
	        			// if path to otherNode is longer, change longest path to that path
						if (otherNode.weightedCostFromStartVertex > longest.weightedCostOfPath) {
							longest = getPath(otherNode.name);
						}
					}
				}
